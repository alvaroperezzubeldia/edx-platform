...
def get_retired_username_by_username(username):
    """
    Wrapper around user_utils hashing functions to incorporate settings
    """
    return user_util.get_retired_username(username, settings.RETIRED_USERNAME_SALTS, settings.RETIRED_USERNAME_FMT)
...
def get_all_retired_usernames_by_username(username):
    ...
    Wrapper around user_utils hashing functions to incorporate settings
    """
    return user_util.get_all_retired_usernames(username, settings.RETIRED_USERNAME_SALTS, settings.RETIRED_USERNAME_FMT)
...
def get_potentially_retired_user_by_username_and_hash(username, hashed_username):
    ...
    - Confirm that a locally hashed username matches the passed in one
      in case of salt mismatches with the upstream script.
    - Attempt to return a User object based on the username, or if it
      doesn't exist, the hashed username (to facilitate retries if
      a retirement API call failed after the username had already been
      hashed.
    """
